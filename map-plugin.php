 ...
 if(!class_exists('Map-Plugin')){
     class Map_Plugin{
         function __construct(){
             ...
             add_filter('manage_map_posts_columns',array($this,'custome_column'));
             add_action('manage_map_posts_custom_column', array($this,'custome_column_content'),10,2);
             add_shortcode('map_shortcode', array($this,'shortcode_generator'));
        ...
        function map_display(){
             add_meta_box('map_display_section',_('Map Section'),array($this, 'map_display_setting'));
         }
        ...
        function save_post_data($post_id){
            //  var_dump($_POST['map_meta']);die();
             if(isset($_POST['map_meta'])){
                 update_post_meta($post_id,'map_meta',$this->sanitize_array($_POST['map_meta']));//santization is not form in $_POST have to write sanitize function.
             }
         ...
         function custome_column_content($columns, $post_id ){
            ...
            if($columns='shortcode'){
                ...
                // print_r($a);
                // echo("</pre>");
              include(plugin_dir_path(__FIlE__).'custome-column-content.php');                
            }
         }
         function shortcode_generator($atts){
             if(isset($atts)){
                ...
                // var_dump($v);
            ob_start();
            include(plugin_dir_path(__FILE__).'map-frontend-content.php');
         ...
         function menu(){
            add_menu_page('Map', 'Map', 'manage_options', 'Map', array($this, 'Map'));
         }
        function Map(){
            echo "hello";
        }
